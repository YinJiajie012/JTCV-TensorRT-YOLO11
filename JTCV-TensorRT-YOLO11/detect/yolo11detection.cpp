...
Yolo11Detection::Yolo11Detection()
{
    
}

void Yolo11Detection::get_engine(){
    if (_access(trtFile_.c_str(), 0) == 0){
...
void Yolo11Detection::Initialize(const std::string trtFile,
    int gpuId,
    float nmsThresh,
    float confThresh,
    int numClass)
...
std::vector<Detection> Yolo11Detection::inference(cv::Mat& img){
    if (img.empty()) return {};
    ...
    int o0 = (int)outputData[0];
    int count = o0 < kMaxNumOutputBbox ? o0 : kMaxNumOutputBbox;
...
void Yolo11Detection::draw_image(cv::Mat& img, std::vector<Detection>& inferResult){
...
YOLO11* Connect::Create_YOLO11_Object()
{
        return new Yolo11Detection;		//注意此处
}
 
 
void Connect::Delete_YOLO11_Object(YOLO11* _bp)
{
        if (_bp)
                delete _bp;
}